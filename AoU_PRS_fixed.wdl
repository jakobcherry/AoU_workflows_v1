...
workflow AoU_PRS {
    ...
    input {
        ...
        # 0 = all chromosomes; 1-22 = one chromosome
        Int chromosome_selector = 0
    }
    ...
    Array[Int] chromosome_indices = if (chromosome_selector == 0) then range(22) else [chromosome_selector - 1]
...
task RunChromosomePRS {
    ...
    command <<<
        ...
        if [[ "$AMBIGUOUS" -gt 0 ]]; then
            echo "WARNING: Ambiguous AoU BIM matches were excluded from scoring."
        fi


        # ==========================================================
        # REMOVE DUPLICATE AoU VARIANT IDs, KEEPING FIRST OCCURRENCE
        # ==========================================================

        awk -F'\t' 'NR == 1 { print; next } !seen[$1]++ { print }' \
            "chr~{chromosome}_aou_prs_score.tsv" > score_dedup.tsv
        mv score_dedup.tsv "chr~{chromosome}_aou_prs_score.tsv"

        awk '!seen[$0]++ { print }' \
            "chr~{chromosome}_prs_extract.txt" > extract_dedup.txt
        mv extract_dedup.txt "chr~{chromosome}_prs_extract.txt"

        # Recalculate the retained matched count after ID deduplication.
        MATCHED=$(($(wc -l < "chr~{chromosome}_aou_prs_score.tsv") - 1))
        ...
        if [[ "$DUPLICATES" -gt 0 ]]; then
            echo "ERROR: Duplicate IDs remain after first-occurrence deduplication."
            exit 1
        fi
