...
# AoU CHRONOTYPE PRS
#
# Scores the existing AoU v9 chromosome-specific PLINK BED/BIM/FAM
# files against the chronotype PRS weight file.
#
# PLINK2 is explicitly staged from GCS rather than assumed to be
# installed in the Docker image.
#
# Persistent diagnostics are uploaded periodically to GCS so that
# diagnostic information remains available even if a task is stopped
# before Cromwell can delocalize normal task outputs.
# ====================================================================
...
workflow AoU_chronotype_PRS {
    ...
    input {
        ...
        # PLINK2 EXECUTABLE
        #
        # This is the exact PLINK2 binary tested successfully on the
        # Dataproc/Workbench VM:
        #
        # PLINK v2.0.0-a.6.9LM 64-bit Intel (29 Jan 2025)
        #
        # It is now stored in GCS so Cromwell can localize it into
        # every Batch shard.
        # ============================================================
...
task RunChromosomePRS {
    ...
    command <<<
        ...
        # PLINK2 executable
        #
        # The executable is staged by Cromwell as ~{plink2_file}.
        # Do NOT depend on plink2 being installed in the Docker image.
        # ============================================================

        echo ""
        echo "============================================================"
        echo "PLINK2 EXECUTABLE"
        echo "============================================================"
        ...
        # Diagnostic uploader
        #
        # This requires gsutil to be available in the runtime
        # environment. Upload failures are recorded explicitly rather
        ...
        upload_diagnostic_log() {
            ...
            echo ""
            echo "--- Diagnostic upload attempt: $(date -u) ---"
            ...
            if command -v gsutil >/dev/null 2>&1; then
                ...
                if gsutil -q cp "$LOG" "${DEBUG_BUCKET}/chr~{chromosome}.log"; then
                    echo "Diagnostic upload SUCCESS"
                else
                    echo "Diagnostic upload FAILED"
                fi

            else

                echo "WARNING: gsutil is NOT available in this container."
        ...
        df -h /mnt/disks/cromwell_root || true
        ...
        # RESOURCE MONITOR
        #
        # Runs independently while PLINK2 is executing.
        #
        # Every 30 seconds records:
        #   - memory
        #   - disk
        #   - load
        #   - PLINK2 processes
